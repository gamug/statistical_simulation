...
def plot_hist_ies(wb):
    ...
    path_p=os.path.join(path,"graph","histograms_"+variable_name)
    ...
    ax.set_title(wb.sheets("distr_lab").range("F2").value+s)
...
def plot_hist():
    ...
    path_p=os.path.join(path,"graph","histograms_"+wb2.sheets("distr_lab").range("F3").value)
    ...
    if wb2.sheets("distr_lab").range("F5").value=="No":
        ...
        for distri in aprox.keys():
            ...
            error=0
        statusbar("recommending functions based on distribution fitted")
        ...
        for i in range(7):
            ...
            wb3.sheets("recommend").pictures.add(path_p
                                                 + r"\try_distr_" + distr_fited + ".jpeg", top=rng.top, left=rng.left)
